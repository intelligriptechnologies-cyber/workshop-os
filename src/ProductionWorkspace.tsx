...
export function ProductionWorkspace({ children }: { children: ReactNode }) {
  ...
  return <div className={`ws-workspace${collapsed?" is-collapsed":""}${drawerOpen?" drawer-open":""}`} data-ui-system="workshopos">
    ...
    <div className="ws-stage"><header className="ws-account-header"><div><strong>{tenant.name}</strong><span>Production workspace</span></div><div className="ws-account"><span className="ws-account-avatar"><UserRound/></span><span><strong>{membership.displayName}</strong><small>{membership.email||"Authenticated member"}</small></span><button onClick={logout}><LogOut size={17}/>Logout</button></div></header><div className="ws-route-content">{children}</div></div>
  </div>;
}
