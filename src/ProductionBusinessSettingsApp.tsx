...
function SettingsScreen({ auth, session }: { auth: SettingsAuth; session: Awaited<ReturnType<ReturnType<typeof createProductionBusinessSettingsApi>["session"]>> }) {
  ...
  return <main className="v12-settings"><header><a href="/">Back to WorkshopOS</a><h1>Business Settings</h1><p>Publish tenant defaults and branch overrides without changing active work.</p><ProductionNavigation permissions={session.membership.permissions} /></header>
    ...
    <section aria-label="Settings editor"><div role="tablist" aria-label="Business Settings sections">{(["operations", "billing", "communications"] as Tab[]).map((item) => <button type="button" role="tab" aria-selected={tab === item} aria-controls={`settings-${item}`} id={`tab-${item}`} key={item} onClick={() => setTab(item)}>{item[0].toUpperCase() + item.slice(1)}</button>)}</div>
      ...
      <div className="v12-settings-actions"><button type="button" disabled={busy || !workspace} onClick={() => void save()}>Save draft</button><button type="button" disabled={busy || !workspace} onClick={() => void publish()}>Publish settings</button><button type="button" disabled={busy} onClick={() => void load()}>Refresh</button></div>
    </section></main>;
}
