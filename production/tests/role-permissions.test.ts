...
test("authorized administrators create custom roles from the hierarchical permission catalog", async () => {
  ...
  assert.deepEqual(directory.catalog.flatMap((group) => group.pages).flatMap((page) => page.actions).map((item) => item.key), ["membership.manage", "inventory.export", "customer.export", "vehicle.export", "work-item.export"]);
