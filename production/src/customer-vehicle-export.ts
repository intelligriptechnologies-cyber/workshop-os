...
export function createCustomerVehicleExportArtifact(screen: "customers" | "vehicles", format: "PDF" | "XLSX", rows: CustomerRecord[] | VehicleRecord[]) {
  ...
  if (format === "XLSX") return { content: Buffer.from(writeObjectXlsx(table, screen)), rowCount: rows.length, filename: `${screen}-${stamp}.xlsx`, mimeType: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" };
