...
export function createInventoryExportArtifact(format: "PDF" | "XLSX", rows: InventoryPosition[]) {
  ...
  if (format === "XLSX") return { content: Buffer.from(writeObjectXlsx(table, "inventory")), rowCount: rows.length, filename: `inventory-${stamp}.xlsx`, mimeType: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" };
