...
import type { RemainingScreenKey, RemainingScreenRow } from "../src/remaining-screens.js";
...
export type RemainingScreenListResult = { rows: RemainingScreenRow[]; page: { page: number; pageSize: number; totalCount: number; pageCount: number }; query: ServerListQuery };
...
export class PostgresVertical implements AdminUserRepository, RolePermissionRepository, BusinessSettingsRepository {
  ...
  async queryRemainingScreen(membership: Membership, screen: RemainingScreenKey, query: ServerListQuery, all = false): Promise<RemainingScreenListResult> {
    if (query.branchId && !membership.branchIds.includes(query.branchId)) throw new ApiError(403, "BRANCH_FORBIDDEN");
    const sources: Record<RemainingScreenKey, string> = {
      appointments: `SELECT a.id::text,a.branch_id::text,coalesce(c.display_name,'Appointment') title,concat(v.normalized_registration,' · ',to_char(a.scheduled_start,'YYYY-MM-DD HH24:MI')) subtitle,a.status,a.updated_at,a.resource_version version FROM workshopos.appointment a JOIN workshopos.customer c ON c.id=a.customer_id AND c.tenant_id=a.tenant_id AND c.branch_id=a.branch_id JOIN workshopos.vehicle v ON v.id=a.vehicle_id AND v.tenant_id=a.tenant_id AND v.branch_id=a.branch_id`,
      "follow-ups": `SELECT f.id::text,f.branch_id::text,f.description title,concat('Job ',f.job_id::text,' · due ',to_char(f.due_at,'YYYY-MM-DD HH24:MI')) subtitle,f.status,coalesce(f.completed_at,f.created_at) updated_at,f.resource_version version FROM workshopos.advisor_follow_up f`,
      "action-inbox": `SELECT a.id::text,a.branch_id::text,a.title,concat(a.next_action,coalesce(' · '||a.blocker,'')) subtitle,a.status,coalesce(a.completed_at,a.created_at) updated_at,a.resource_version version FROM workshopos.action_inbox a`,
      materials: `SELECT l.id::text,r.branch_id::text,concat('Job ',r.job_id::text) title,concat(i.sku,' · ',l.requested_quantity::text,' ',l.uom,' requested · ',l.issued_quantity::text,' issued') subtitle,r.status,r.created_at updated_at,r.resource_version version FROM workshopos.job_material_request r JOIN workshopos.job_material_request_line l ON l.request_id=r.id AND l.tenant_id=r.tenant_id AND l.branch_id=r.branch_id JOIN workshopos.inventory_item i ON i.id=l.item_id AND i.tenant_id=l.tenant_id AND i.branch_id=l.branch_id`,
      reports: `SELECT f.id::text,f.branch_id::text,replace(initcap(f.category),'_',' ') title,concat(f.metric_key,' = ',f.metric_value::text,' · ',f.source_ledger_ref) subtitle,'RECORDED' status,f.recorded_at updated_at,1::bigint version FROM workshopos.report_fact f`,
      masters: `SELECT 'warehouse:'||w.id::text id,w.branch_id::text,w.name title,'Inventory warehouse' subtitle,CASE WHEN w.active THEN 'ACTIVE' ELSE 'INACTIVE' END status,transaction_timestamp() updated_at,w.resource_version version FROM workshopos.inventory_warehouse w UNION ALL SELECT 'item:'||i.id::text,i.branch_id::text,i.sku,'Inventory item · '||i.base_uom,CASE WHEN i.active THEN 'ACTIVE' ELSE 'INACTIVE' END,transaction_timestamp(),i.resource_version FROM workshopos.inventory_item i UNION ALL SELECT 'supplier:'||s.id::text,s.branch_id::text,s.legal_name,'Supplier · '||s.currency,s.status,transaction_timestamp(),s.resource_version FROM workshopos.supplier s UNION ALL SELECT 'configuration:'||m.id::text,m.branch_id::text,m.master_key,'Configuration · '||m.master_type,'CONFIGURED',m.created_at,1::bigint FROM workshopos.configuration_master m UNION ALL SELECT 'capacity:'||r.id::text,r.branch_id::text,r.resource_name,'Capacity resource · '||r.resource_kind,CASE WHEN r.effective_to IS NULL OR r.effective_to>transaction_timestamp() THEN 'ACTIVE' ELSE 'INACTIVE' END,r.effective_from,1::bigint FROM workshopos.appointment_capacity_resource r`,
    };
    return this.inScope(membership, async (client) => {
      const values: unknown[] = [membership.branchIds]; const where = ["source.branch_id=ANY($1::text[])"];
      if (query.branchId) { values.push(query.branchId); where.push(`source.branch_id=$${values.length}`); }
      if (query.search) { values.push(`%${query.search}%`); where.push(`(source.title ILIKE $${values.length} OR source.subtitle ILIKE $${values.length} OR source.status ILIKE $${values.length})`); }
      const from = `(${sources[screen]}) source`; const condition = where.join(" AND ");
      const totalCount = Number((await client.query<{ count: string }>(`SELECT count(*)::text count FROM ${from} WHERE ${condition}`, values)).rows[0].count);
      const pageCount = Math.max(1, Math.ceil(totalCount / query.pageSize)); const page = all ? 1 : Math.min(query.page, pageCount);
      const orders: Record<string,string> = { "summary.asc": "lower(source.title) ASC,source.id", "summary.desc": "lower(source.title) DESC,source.id", "updatedAt.asc": "source.updated_at ASC,source.id", "updatedAt.desc": "source.updated_at DESC,source.id" };
      const paging = all ? "" : ` LIMIT ${query.pageSize} OFFSET ${(page - 1) * query.pageSize}`;
      const result = await client.query<any>(`SELECT source.* FROM ${from} WHERE ${condition} ORDER BY ${orders[query.sort]}${paging}`, values);
      return { rows: result.rows.map((row) => ({ id: row.id, branchId: row.branch_id, title: row.title, subtitle: row.subtitle, status: row.status, updatedAt: new Date(row.updated_at).toISOString(), version: Number(row.version) })), page: { page, pageSize: query.pageSize, totalCount, pageCount }, query: { ...query, page } };
    });
  }

  async completeRemainingScreenRow(membership: Membership, screen: "follow-ups" | "action-inbox", id: string, input: { version: number; reason: string }, key: string) {
    if (!membership.id || !input.reason.trim()) throw new ApiError(input.reason.trim() ? 403 : 422, input.reason.trim() ? "MEMBERSHIP_REQUIRED" : "REASON_REQUIRED");
    return this.inScope(membership, async (client) => {
      const table = screen === "follow-ups" ? "advisor_follow_up" : "action_inbox";
      const row = (await client.query<any>(`SELECT id,branch_id,status,resource_version FROM workshopos.${table} WHERE id=$1 FOR UPDATE`, [id])).rows[0];
      if (!row) throw new ApiError(404, "RECORD_NOT_FOUND");
      return this.operationalCommand(client, membership, row.branch_id, key, { screen, id, input }, async () => {
        if (row.status !== "OPEN") throw new ApiError(409, "RECORD_NOT_ACTIONABLE");
        if (Number(row.resource_version) !== input.version) throw new ApiError(409, "VERSION_CONFLICT");
        if (screen === "follow-ups") {
          await client.query("UPDATE workshopos.advisor_follow_up SET status='COMPLETED',outcome=$1,completed_at=transaction_timestamp(),resource_version=resource_version+1 WHERE id=$2", [input.reason.trim(), id]);
          await client.query("INSERT INTO workshopos.advisor_follow_up_history(id,tenant_id,branch_id,follow_up_id,action,owner_identity_id,due_at,outcome,actor_identity_id,membership_id,occurred_at,audit_reference) SELECT $1,tenant_id,branch_id,id,'COMPLETED',owner_identity_id,due_at,$2,$3,$4,transaction_timestamp(),$5 FROM workshopos.advisor_follow_up WHERE id=$6", [randomUUID(), input.reason.trim(), this.actorId(membership), membership.id, randomUUID(), id]);
        } else await client.query("UPDATE workshopos.action_inbox SET status='COMPLETED',completed_at=transaction_timestamp(),resource_version=resource_version+1 WHERE id=$1", [id]);
        return { row: { id, branchId: row.branch_id, title: "Completed", subtitle: input.reason.trim(), status: "COMPLETED", updatedAt: new Date().toISOString(), version: input.version + 1 } };
      });
    });
  }
