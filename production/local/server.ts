...
function queueRemainingScreenExport(membership:Parameters<PostgresVertical["queryRemainingScreen"]>[0],id:string,screen:RemainingScreenKey,format:"PDF"|"XLSX",query:ServerListQuery):void{setImmediate(()=>void(async()=>{try{const rows=(await database.queryRemainingScreen(membership,screen,query,true)).rows;await database.completeListExport(membership,id,createRemainingScreenExportArtifact(screen,format,rows));}catch{await database.failListExport(membership,id);}})());}
...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    if (url.pathname.startsWith("/api/")) {
      ...
      const remainingListRoute = url.pathname.match(/^\/api\/v1\/operations\/([a-z-]+)$/);
      if (remainingListRoute && request.method === "GET" && isRemainingScreenKey(remainingListRoute[1])) { const screen=remainingListRoute[1]; requirePermission(membership,REMAINING_SCREEN_META[screen].readPermission); json(response,200,await database.queryRemainingScreen(membership,screen,parseListQuery(url.searchParams)),traceId); return; }
      const remainingCompleteRoute = url.pathname.match(/^\/api\/v1\/operations\/(follow-ups|action-inbox)\/([^/]+)\/complete$/);
      if (remainingCompleteRoute && request.method === "POST") { const screen=remainingCompleteRoute[1] as "follow-ups"|"action-inbox"; requirePermission(membership,REMAINING_SCREEN_META[screen].managePermission!); const input=await body(request); const result=await database.completeRemainingScreenRow(membership,screen,remainingCompleteRoute[2],{version:Number(input.version),reason:String(input.reason??"")},String(request.headers["idempotency-key"]??"")); json(response,result.replay?200:201,{row:result.row},traceId); return; }
      const remainingPreference = url.pathname.match(/^\/api\/v1\/list-preferences\/([a-z-]+)$/);
      if (remainingPreference && isRemainingScreenKey(remainingPreference[1])) { const screen=remainingPreference[1]; requirePermission(membership,REMAINING_SCREEN_META[screen].readPermission); if(request.method==="GET"){json(response,200,{preference:await database.getListPreference(membership,screen)},traceId);return;} if(request.method==="PUT"){const input=await body(request),viewMode=String(input.viewMode??"");if(viewMode!=="grid"&&viewMode!=="table")throw new ApiError(400,"VIEW_MODE_INVALID");json(response,200,{preference:await database.saveListPreference(membership,screen,viewMode)},traceId);return;} }
      if (url.pathname === "/api/v1/operation-exports" && request.method === "POST") { const input=await body(request),screen=String(input.screen??"");if(!isRemainingScreenKey(screen))throw new ApiError(400,"EXPORT_SCREEN_INVALID");requirePermission(membership,`${screen}.export`);const format=String(input.format??"").toUpperCase();if(format!=="PDF"&&format!=="XLSX")throw new ApiError(400,"EXPORT_FORMAT_INVALID");const query=queryFromJson(input.query),result=await database.createListExport(membership,screen,format,query,String(request.headers["idempotency-key"]??""));if(!result.replay||result.job.status==="PENDING")queueRemainingScreenExport(membership,result.job.id,screen,format,query);json(response,result.replay?200:202,{export:result.job},traceId);return; }
      const operationExportRoute=url.pathname.match(/^\/api\/v1\/operation-exports\/([0-9a-f-]+)$/i);if(operationExportRoute&&request.method==="GET"){const job=await database.getListExport(membership,operationExportRoute[1]);if(!isRemainingScreenKey(job.screenKey))throw new ApiError(404,"EXPORT_NOT_FOUND");requirePermission(membership,`${job.screenKey}.export`);json(response,200,{export:job},traceId);return;}
      const operationExportDownload=url.pathname.match(/^\/api\/v1\/operation-exports\/([0-9a-f-]+)\/download$/i);if(operationExportDownload&&request.method==="GET"){const job=await database.getListExport(membership,operationExportDownload[1]);if(!isRemainingScreenKey(job.screenKey))throw new ApiError(404,"EXPORT_NOT_FOUND");requirePermission(membership,`${job.screenKey}.export`);const artifact=await database.downloadListExport(membership,job.id);response.writeHead(200,{"content-type":artifact.mimeType,"content-disposition":`attachment; filename="${artifact.filename.replace(/["\r\n]/g,"")}"`,"cache-control":"private, no-store"});response.end(artifact.content);return;}
