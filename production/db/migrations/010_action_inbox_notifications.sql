...
BEGIN
  ...
  WITH candidates AS (
    ...
    WHERE tenant_id = workshopos.current_tenant_id()::text
      AND branch_id = ANY (workshopos.authorized_branch_ids()::text[])
...
CREATE POLICY notification_domain_event_tenant_isolation ON workshopos.notification_domain_event USING (tenant_id = workshopos.current_tenant_id()::text AND branch_id = ANY (workshopos.authorized_branch_ids()::text[])) WITH CHECK (tenant_id = workshopos.current_tenant_id()::text AND branch_id = ANY (workshopos.authorized_branch_ids()::text[]));
CREATE POLICY action_inbox_tenant_isolation ON workshopos.action_inbox USING (tenant_id = workshopos.current_tenant_id()::text AND branch_id = ANY (workshopos.authorized_branch_ids()::text[])) WITH CHECK (tenant_id = workshopos.current_tenant_id()::text AND branch_id = ANY (workshopos.authorized_branch_ids()::text[]));
CREATE POLICY notification_template_tenant_isolation ON workshopos.notification_template USING (tenant_id = workshopos.current_tenant_id()::text AND branch_id = ANY (workshopos.authorized_branch_ids()::text[])) WITH CHECK (tenant_id = workshopos.current_tenant_id()::text AND branch_id = ANY (workshopos.authorized_branch_ids()::text[]));
CREATE POLICY notification_recipient_tenant_isolation ON workshopos.notification_recipient USING (tenant_id = workshopos.current_tenant_id()::text AND branch_id = ANY (workshopos.authorized_branch_ids()::text[])) WITH CHECK (tenant_id = workshopos.current_tenant_id()::text AND branch_id = ANY (workshopos.authorized_branch_ids()::text[]));
CREATE POLICY notification_preference_history_tenant_isolation ON workshopos.notification_preference_history USING (tenant_id = workshopos.current_tenant_id()::text AND branch_id = ANY (workshopos.authorized_branch_ids()::text[])) WITH CHECK (tenant_id = workshopos.current_tenant_id()::text AND branch_id = ANY (workshopos.authorized_branch_ids()::text[]));
CREATE POLICY notification_outbox_tenant_isolation ON workshopos.notification_outbox USING (tenant_id = workshopos.current_tenant_id()::text AND branch_id = ANY (workshopos.authorized_branch_ids()::text[])) WITH CHECK (tenant_id = workshopos.current_tenant_id()::text AND branch_id = ANY (workshopos.authorized_branch_ids()::text[]));
CREATE POLICY in_app_notification_tenant_isolation ON workshopos.in_app_notification USING (tenant_id = workshopos.current_tenant_id()::text AND branch_id = ANY (workshopos.authorized_branch_ids()::text[])) WITH CHECK (tenant_id = workshopos.current_tenant_id()::text AND branch_id = ANY (workshopos.authorized_branch_ids()::text[]));
CREATE POLICY notification_delivery_attempt_tenant_isolation ON workshopos.notification_delivery_attempt USING (tenant_id = workshopos.current_tenant_id()::text AND branch_id = ANY (workshopos.authorized_branch_ids()::text[])) WITH CHECK (tenant_id = workshopos.current_tenant_id()::text AND branch_id = ANY (workshopos.authorized_branch_ids()::text[]));
CREATE POLICY provider_status_event_tenant_isolation ON workshopos.provider_status_event USING (tenant_id = workshopos.current_tenant_id()::text AND branch_id = ANY (workshopos.authorized_branch_ids()::text[])) WITH CHECK (tenant_id = workshopos.current_tenant_id()::text AND branch_id = ANY (workshopos.authorized_branch_ids()::text[]));
CREATE POLICY notification_dead_letter_replay_tenant_isolation ON workshopos.notification_dead_letter_replay USING (tenant_id = workshopos.current_tenant_id()::text AND branch_id = ANY (workshopos.authorized_branch_ids()::text[])) WITH CHECK (tenant_id = workshopos.current_tenant_id()::text AND branch_id = ANY (workshopos.authorized_branch_ids()::text[]));
CREATE POLICY notification_audit_tenant_isolation ON workshopos.notification_audit USING (tenant_id = workshopos.current_tenant_id()::text AND branch_id = ANY (workshopos.authorized_branch_ids()::text[])) WITH CHECK (tenant_id = workshopos.current_tenant_id()::text AND branch_id = ANY (workshopos.authorized_branch_ids()::text[]));
