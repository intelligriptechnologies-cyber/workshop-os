...
async function platformApi(path, identity, init = {}) {
  return fetch(`${baseUrl}${path}`, {
    ...init,
    headers: { "content-type": "application/json", "x-workshopos-platform-identity": identity, "x-workshopos-platform-mfa-at": new Date().toISOString(), ...(init.headers ?? {}) },
  });
}
...
assert.equal(health.migrations, 45);

assert.equal((await api("/api/v1/platform/session", "north-admin")).status, 401);
const platformSessionResponse=await platformApi("/api/v1/platform/session","platform-admin");assert.equal(platformSessionResponse.status,200);const platformSession=await platformSessionResponse.json();assert.equal(platformSession.principal.identityId,"platform-admin");assert.ok(platformSession.tenants.some(tenant=>tenant.id==="00000000-0000-4000-8000-000000000001"));
const platformTenant=await platformApi("/api/v1/platform/tenants/00000000-0000-4000-8000-000000000001","support-agent").then(response=>response.json());assert.ok(platformTenant.branches.some(item=>item.id===branch));assert.ok(platformTenant.users.some(item=>item.id==="00000000-0000-4000-8000-000000000104"));
const supportGrantResponse=await platformApi("/api/v1/platform/support-grants","platform-admin",{method:"POST",body:JSON.stringify({tenantId:"00000000-0000-4000-8000-000000000001",supportIdentityId:"support-agent",branchIds:[branch],permissions:["tenant.emulate"],reason:"Local scoped support verification",expiresAt:new Date(Date.now()+60*60_000).toISOString()})});assert.equal(supportGrantResponse.status,201);const supportGrant=(await supportGrantResponse.json()).grant;
assert.equal((await platformApi(`/api/v1/platform/support-grants/${supportGrant.id}/approve`,"platform-admin",{method:"POST",body:JSON.stringify({reason:"Self approval forbidden"})})).status,403);
assert.equal((await platformApi(`/api/v1/platform/support-grants/${supportGrant.id}/approve`,"platform-approver",{method:"POST",body:JSON.stringify({reason:"Independent scope approval"})})).status,200);
const emulationRequestResponse=await platformApi("/api/v1/platform/emulations","support-agent",{method:"POST",body:JSON.stringify({grantId:supportGrant.id,tenantId:"00000000-0000-4000-8000-000000000001",membershipId:"00000000-0000-4000-8000-000000000104",branchId:branch,reason:"Reproduce tenant user report"})});assert.equal(emulationRequestResponse.status,201);const emulation=(await emulationRequestResponse.json()).emulation;
assert.equal((await platformApi(`/api/v1/platform/emulations/${emulation.id}/approve`,"support-agent",{method:"POST",body:JSON.stringify({reason:"Self approval forbidden"})})).status,403);
assert.equal((await platformApi(`/api/v1/platform/emulations/${emulation.id}/approve`,"platform-approver",{method:"POST",body:JSON.stringify({reason:"Exact tenant user and branch verified"})})).status,200);
const startedEmulation=await platformApi(`/api/v1/platform/emulations/${emulation.id}/start`,"support-agent",{method:"POST",body:"{}"}).then(response=>response.json()).then(value=>value.emulation);assert.equal(Date.parse(startedEmulation.expiresAt)-Date.parse(startedEmulation.startedAt),15*60_000);
const emulationHeaders={"x-workshopos-platform-identity":"support-agent","x-workshopos-platform-mfa-at":new Date().toISOString(),"x-workshopos-emulation-id":emulation.id};const emulatedSessionResponse=await fetch(`${baseUrl}/api/v1/session`,{headers:emulationHeaders});assert.equal(emulatedSessionResponse.status,200);const emulatedSession=await emulatedSessionResponse.json();assert.equal(emulatedSession.membership.id,"00000000-0000-4000-8000-000000000104");assert.deepEqual(emulatedSession.membership.branchIds,[branch]);assert.ok(!emulatedSession.membership.permissions.some(permission=>permission.startsWith("platform.")));
const emulatedDenied=await fetch(`${baseUrl}/api/v1/jobs`,{headers:emulationHeaders});assert.equal(emulatedDenied.status,403);const deniedTrace=emulatedDenied.headers.get("x-trace-id");for(let attempt=0;attempt<20;attempt+=1){const audit=await platformApi("/api/v1/platform/audit","platform-approver").then(response=>response.json());if(audit.emulatedActions.some(action=>action.trace_id===deniedTrace&&action.platform_actor_id==="support-agent"&&action.effective_membership_id==="00000000-0000-4000-8000-000000000104"))break;if(attempt===19)assert.fail("dual-attribution audit was not persisted");await new Promise(resolve=>setTimeout(resolve,50));}
const platformLogs=await platformApi("/api/v1/platform/logs","platform-admin").then(response=>response.json()).then(value=>value.logs);const onlineLog=platformLogs.find(log=>log.tier==="ONLINE"),recoverableLog=platformLogs.find(log=>log.tier==="RECOVERABLE"&&!log.restored);assert.ok(onlineLog);assert.ok(recoverableLog);const protectedDownload=await platformApi(`/api/v1/platform/logs/${onlineLog.id}/download?reason=Local%20incident%20evidence`,"platform-admin");assert.equal(protectedDownload.status,200);assert.match(protectedDownload.headers.get("cache-control")??"",/private/);assert.equal((await platformApi(`/api/v1/platform/logs/${recoverableLog.id}/download?reason=Before%20recovery`,"platform-admin")).status,409);assert.equal((await platformApi(`/api/v1/platform/logs/${recoverableLog.id}/recover`,"platform-admin",{method:"POST",body:JSON.stringify({reason:"Local checksum recovery exercise"})})).status,201);assert.equal((await platformApi(`/api/v1/platform/logs/${recoverableLog.id}/download?reason=After%20recovery`,"platform-admin")).status,200);
